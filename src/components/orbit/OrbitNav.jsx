...
{ id: "britain", label: "Britain", icon: BritainIcon, to: "/events", ctx: "britain" },
...
{ id: "britain", label: "Britain", icon: BritainIcon, to: "/org/events", ctx: "britain" },
...
function resolveCtx(to, ctx) {
  if (!ctx) return to;
  const lastEvent = sessionStorage.getItem("orbit:lastEvent");
  const ev = (s, f) => lastEvent ? `/events/${lastEvent}${s}` : f;
  switch (ctx) {
...
const SLOTS = [20, 45, 70];
const SPACING = 25;
const ICON_R_RATIO = 0.78;
const SCROLL_THRESHOLD = 30;
const deg2rad = (d) => d * Math.PI / 180;

function arcPos(alphaDeg, radius, panelSize) {
...
export function OrbitNav() {
  ...
  // Scroll to rotate (only when open)
  useEffect(() => {
    if (collapsed) return;
    const onWheel = (e) => {
      e.preventDefault();
  ...
  const positioned = useMemo(() => {
    return items.map((item, i) => {
      ...
      let itemScale = 0.75;
      let isActive = false;
      if (raw === 0) {slotAngle = SLOTS[0];opacity = 0.6;itemScale = 0.88;} else
      if (raw === 1) {slotAngle = SLOTS[1];opacity = 1;itemScale = 1;isActive = true;} else
      if (raw === 2) {slotAngle = SLOTS[2];opacity = 0.6;itemScale = 0.88;} else
      {slotAngle = SLOTS[2] + SPACING;opacity = 0;itemScale = 0.6;}
  ...
  return (
    <>
      {/* ============================================ */}
      {/* TOGGLE BUTTON — always rendered, always visible */}
      {/* ============================================ */}
      <button
        type="button"
        ...
        style={{
          boxShadow: collapsed ?
          "0 4px 16px -4px rgba(255,92,56,0.35), 0 0 10px -2px rgba(255,92,56,0.2)" :
          "0 6px 24px -4px rgba(255,92,56,0.45), 0 0 16px -2px rgba(255,92,56,0.25)"
        }}>
        
        {ripples.map((t) =>
        <span key={t} className="orb-ripple absolute inset-0 rounded-full" />
        )}
        {!collapsed && <span className="absolute inset-[3px] rounded-full border border-ember/20" />}
        <span
      ...
      {/* ============================================ */}
      {/* QUARTER-CIRCLE PANEL — only renders when expanded */}
      {/* ============================================ */}
      {!collapsed &&
      <div
        ref={panelRef}
