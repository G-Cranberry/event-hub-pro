...
export function CardCarousel({ events }) {
  ...
  function handlePointerUp() {
    const threshold = 50;
    if (dragDelta.current > threshold) prev();else
  ...
  function handleWheel(e) {
    if (Math.abs(e.deltaX) > Math.abs(e.deltaY)) {
      if (e.deltaX > 30) next();else
      if (e.deltaX < -30) prev();
    } else {
      if (e.deltaY > 30) next();else
      if (e.deltaY < -30) prev();
    }
  }
          ...
          return (
            <div key={event._id} className={`card-carousel__slot card-carousel__slot--${slot}`}>
              <div
                ...
                style={{
                  ["--card-glow"]: `${event.accent}55`,
                  ["--card-bg"]: `linear-gradient(160deg, #1a1028 0%, #120a1e 100%)`
                }}>
...
export function CinematicBg({ children }) {
  return (
