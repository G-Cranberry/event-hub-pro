...
export default function OrgScanner() {
  ...
  const handleScan = async (raw) => {
    ...
    try {
      const found = await lookup({ qrData, eventId: id });
      if (!found) {
        setResult(null);
  ...
  const handleManual = async (e) => {
    ...
    setLookupError(null);
    try {
      const found = await lookup({ qrData: manual.trim(), eventId: id });
      if (!found) {
  ...
  const toggleCheckIn = async () => {
    ...
    try {
      ...
      const found = await lookup({ qrData: result.registration.qrData, eventId: id });
