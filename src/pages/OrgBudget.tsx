...
export default function OrgBudget() {
  const { id = "" } = useParams();
  const entries = useQuery(api.budgets.listByEvent, { eventId: id as any });
  ...
  const [showForm, setShowForm] = useState<false | "expense" | "sponsor">(false);
  ...
  const stats = useMemo(() => {
    if (!entries) return { totalExpenses: 0, totalSponsors: 0, balance: 0, expenseCount: 0, sponsorCount: 0, byCategory: {} as Record<string, number> };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (showForm === "expense") {
        await addExpense({ eventId: id as any, label: label.trim(), amount: num, category, note: note.trim() || undefined });
        toast.success("Expense logged");
      } else {
        if (!sponsorName.trim()) { toast.error("Sponsor name required"); setBusy(false); return; }
        await addSponsor({ eventId: id as any, label: label.trim(), amount: num, sponsorName: sponsorName.trim(), note: note.trim() || undefined });
  ...
  const handleDelete = async (entryId: string) => {
    try { await removeEntry({ entryId: entryId as any }); toast.success("Removed"); } catch { toast.error("Failed"); }
  };
