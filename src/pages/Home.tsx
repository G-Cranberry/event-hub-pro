...
/** Organizer snapshot dashboard — minimal, focused */
function OrganizerDashboard() {
  const myEvents = useQuery(api.events.myEvents);
  const now = Date.now();

  const stats = useMemo(() => {
    if (!myEvents) return { total: 0, upcoming: 0, todayRegs: 0 };
    const upcoming = myEvents.filter((e) => e.endDate > now && e.status === "published");
    return { total: myEvents.length, upcoming: upcoming.length, todayRegs: 0 };
  }, [myEvents, now]);

  const featured = useMemo(() => {
    if (!myEvents) return null;
    return myEvents.find((e) => e.endDate > now && e.status === "published") ?? myEvents[0] ?? null;
  }, [myEvents, now]);
  ...
  return (
    <div className="relative z-10 mx-auto max-w-6xl px-5 pt-20 sm:pt-24">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
        ...
        <h1 className="mt-5 font-display text-4xl font-bold leading-[1.06] tracking-tight text-foreground sm:text-5xl lg:text-6xl">
          Your event <span className="text-gradient-ember">command center</span>
        </h1>
        <p className="mt-5 max-w-xl text-[15px] leading-7 text-foreground/60">
          Manage events, scan passes, track budgets, send announcements — all from one place.
        </p>
      </motion.div>

      {/* Stats row */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="mt-8 grid grid-cols-3 gap-3">
        {[
          { label: "Total Events", value: stats.total, icon: LayoutGrid, color: "text-ember" },
          { label: "Upcoming", value: stats.upcoming, icon: CalendarDays, color: "text-accent" },
          { label: "Today's Check-ins", value: stats.todayRegs, icon: CheckCircle2, color: "text-emerald-400" },
        ].map(({ label: l, value: v, icon: Icon, color }) => (
          <div key={l} className="orb-card orb-neon-border p-5">
            <Icon className={`h-5 w-5 ${color}`} />
            <p className={`mt-3 font-display text-3xl font-bold ${color}`}>{v}</p>
      ...
      {featured && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.15 }} className="mt-8">
          <h2 className="mb-4 font-display text-lg font-bold text-foreground">Upcoming Event</h2>
          <Link to={`/org/events/${featured._id}`} className="orb-card orb-neon-border orb-hud-corners group block overflow-hidden p-0">
            <div className="px-6 py-6 sm:px-8">
              <div className="flex items-center justify-between">
                <span className="rounded-full border border-white/15 bg-black/30 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white">
                  {TYPE_LABEL[featured.type]}
                </span>
                <span className={`text-[10px] font-bold uppercase tracking-wider ${featured.regOpen ? "text-accent" : "text-amber-300"}`}>
                  {featured.regOpen ? "● Live" : "Closed"}
                </span>
              </div>
              <h3 className="mt-3 font-display text-2xl font-bold text-white sm:text-3xl">{featured.title}</h3>
              <p className="mt-1 text-sm text-white/55">{fmtRange(featured.startDate, featured.endDate)} · {featured.city}</p>
            </div>
            <div className="flex items-center justify-between border-t border-white/8 px-6 py-3">
              <span className="text-[10px] font-semibold uppercase tracking-wider text-white/40">
                {featured.formSchema.length} form fields · {featured.registrationType} registration
              </span>
              <span className="flex items-center gap-1 text-xs font-semibold text-ember transition-colors group-hover:text-ember/80">
      ...
      {/* Quick links */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {[
          { label: "My Events", to: "/org/events", icon: LayoutGrid, desc: "View and manage all your events" },
          { label: "New Event", to: "/org/events/new", icon: Plus, desc: "Create a new event from scratch" },
          { label: "Profile", to: "/profile", icon: Users, desc: "Account settings and mode switch" },
        ].map(({ label: l, to, icon: Icon, desc }) => (
          <Link key={l} to={to} className="orb-card group flex items-center gap-3.5 p-4 transition-transform hover:-translate-y-0.5">
            ...
            <div className="min-w-0">
              <p className="text-sm font-bold text-foreground">{l}</p>
              <p className="truncate text-[11px] text-foreground/45">{desc}</p>
            </div>
          </Link>
        ))}
      </motion.div>
...
export default function Home() {
  ...
  const isOrganizer = profile?.currentMode === "organizer";
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      {isOrganizer ? (
        ...
        <>
          {/* ─── Participant: Hero with Carousel ─── */}
          ...
          <section className="relative z-10 mx-auto max-w-5xl px-5 py-20">
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              <Link to="/events" className="orb-card orb-hud-corners orb-scanlines group block p-8">
              ...
              <Link to="/org/events" className="orb-card orb-hud-corners orb-scanlines group block p-8">
                <div className="flex items-center gap-3">
        ...
        <div className="mx-auto max-w-6xl">
          <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              ...
              <ul className="space-y-2">
                {[{ label: "Create Event", to: "/org/events/new" }, { label: "My Events", to: "/org/events" }, { label: "Live Dashboard", to: "/org/events" }].map((l) => (
                  <li key={l.label}><Link to={l.to} className="text-sm text-foreground/55 transition-colors hover:text-ember">{l.label}</Link></li>
                ))}
              </ul>
            </div>
