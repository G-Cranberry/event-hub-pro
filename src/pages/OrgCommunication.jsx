...
export default function OrgCommunication() {
  const { id = "" } = useParams();
  const announcements = useQuery(api.announcements.listByEvent, { eventId: id });
  ...
  const handleSend = async () => {
    ...
    setBusy(true);
    try {
      await sendAnnouncement({ eventId: id, title: title.trim(), body: body.trim(), priority });
  ...
  const handleDelete = async (announcementId) => {
    try {await removeAnnouncement({ announcementId: announcementId });toast.success("Deleted");} catch {toast.error("Failed");}
  };
