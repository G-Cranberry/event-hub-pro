...
function useColumnVisibility(eventId: string, allColumns: string[]) {
  ...
  const [hidden, setHidden] = useState<Set<string>>(() => {
    ...
    return new Set<string>(); // all visible by default
  });
  ...
  const handleExport = () => {
    ...
    const rows = filtered.map((r) => {
      ...
      return visible.map((col) => {
        ...
        // form data field
        return String((reg.formData as Record<string, unknown>)?.[col] ?? "");
      });
    });
    ...
    <div className="mx-auto min-h-screen max-w-[1400px] px-5 pb-24 pt-20 sm:pt-24">
      ...
      <div className="mt-6 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-wrap items-center gap-2">
          {(["all", "pending", "attended"] as StatusFilter[]).map((s) => (
            <button
              key={s}
              type="button"
